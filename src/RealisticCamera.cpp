...
bool RealisticCamera::trace(const Ray& in, Ray& out) const {
    ...
    float elementZ = 0.f;
    for (int i = 0; i < interfaces.size(); i++) {
        const LensInterface& interface = interfaces[i];
        elementZ -= interface.thickness;

        // Intersect with element
        bool isStop = interface.curvature == 0.f;
        float t = 0.f;
        glm::vec3 normal;
        if (isStop) t = (elementZ - lensSpace.o.z) / lensSpace.d.z;
        else {
            float z = elementZ + interface.curvature;
